      ...
      case "jurisdiccion":
        return (
          <div key={item.id} className="bg-gray-100 p-3 rounded-md shadow-sm">
            ...
            <p>Fecha Modificacion: {item.fecha_ultima_modificacion || "No disponible"}</p>
          </div>
        );
      case "zona":
        return (
          <div key={item.id} className="bg-gray-100 p-3 rounded-md shadow-sm">
            ...
            <p>Jurisdiccion Asociada: {item.nombre_jurisdiccion || "No disponible"}</p>
          </div>
        );
      case "concesion":
        return (
      ...
      case "ponton":
        return (
          <div key={item.id} className="bg-gray-100 p-3 rounded-md shadow-sm">
            ...
            <p>Ubicacion: {item.ubicacion || "No disponible"}</p>
            <p>Fecha Apertura: {item.fecha_apertura_operacional || "No disponible"}</p>
            <p>Fecha Cierre: {item.fecha_cierre_operacional || "No disponible"}</p>
            <p>Tipo Ponton: {item.tipo_ponton || "No disponible"}</p>
            <p>Habitabilidad General: {item.habitabilidad_general || "No disponible"}</p>
            <p>Habitabilidad Interna: {item.habitabilidad_interna || "No disponible"}</p>
            <p>Habitabilidad Externa: {item.habitabilidad_externa || "No disponible"}</p>
            <p>Concesion Asociada: {item.nombre_concesion || "No disponible"}</p>
          </div>
        );
    {/* CREAR CENTROS DE PRUEBA PARA ASI PODER ASIGNAR PONTONES A EL CENTRO */}
      case "centro":
        return (
          <div key={item.id} className="bg-gray-100 p-3 rounded-md shadow-sm">
